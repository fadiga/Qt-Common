...
from common.util import raise_success, raise_error

DATETIME = u"%s" % unicode(datetime.now().strftime('%d-%m-%Y %Hh%M'))


def export_database_as_file():
    destination = QtGui.QFileDialog.getSaveFileName(QtGui.QWidget(),
                                                    u"Sauvegarder la base de Donnée.",
                                                    u"Sauvegarde du %s %s.db" % (DATETIME, Config.NAME_ORGA),
                                                    "*.db")
    if not destination:
        return None
...
def export_as_excel(name, data):
    try:
        ...
        raise_error("/!\ Import Erreur d'import",
                    " Vérifiez si la fonction 'main_as_excel existe dans le export_xls'")



    destination = QtGui.QFileDialog.getSaveFileName(QtGui.QWidget(),
                                                    u"Save Excel Export as...",
                                                    u"%s %s.xls" % (DATETIME, Config.NAME_ORGA), "*.xls")
    if not destination:
        return False
    try:
